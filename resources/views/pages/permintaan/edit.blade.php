...
@section('content')
	{!! Form::model($permintaan, [
	'method' => 'patch',
	'route' => ['permintaan.update', $permintaan->id]
	]) !!}
...
<div class="row">
  <div class="col-xs-12">
    <div class="box box-primary">
      <div class="box-body">
        <a href="{{route('permintaan.show', $permintaan->id )}}"><button type="button" class="btn btn-default pull-left">Cancel</button></a>
...
<div class="row">
  <div class="col-md-3">
    <div class="box box-primary">
      ...
      <div class="box-body">
				<input type="hidden" name="OldPermintaan" value="{{$permintaan->MintaCode}}">
        <div class="form-group">
          {!! Form::label('MintaCode', 'Minta Code') !!}
          {!! Form::text('MintaCode', $permintaan -> MintaCode, array('id' => 'MintaCode', 'class' => 'form-control', 'autocomplete' => 'off', 'onKeyUp' => 'capital()', 'placeholder' => 'Input Code', 'required')) !!}
        </div>
        <div class="form-group">
          {!! Form::label('Tgl', 'Date') !!}
          <div class="input-group">
            ...
            {!! Form::text('Tgl', $permintaan->Tgl, array('id' => 'Tgl', 'class' => 'form-control', 'autocomplete' => 'off', 'placeholder' => '31/12/2000', 'required')) !!}
          </div>
        </div>
        <div class="form-group">
          {!! Form::label('Supplier Code', 'Supplier Code') !!}
          {!! Form::text('SCode', $permintaan -> SCode, array('class' => 'form-control', 'id' => 'SCode', 'placeholder' => 'ABC01', 'autocomplete' => 'off', 'onKeyUp' => 'capital()', 'maxlength' => '5', 'required')) !!}
          ...
          <thead>
					...
					<tbody>
						<tr class='tr_input'>
							...
							<td><input type='text' name='Barang[]' id='Barang_0' value='{{$permintaanlists[0]->Barang}}' class='form-control input-sm Barang' placeholder='Main Frame' autocomplete required></td>
							<td><input type='text' name='ICode[]' id='ICode_0' value='{{$permintaanlists[0]->ICode}}' class='form-control input-sm ICode' readonly></td>
							<td><input type='number' name='Quantity[]' id='Quantity_0' value='{{$permintaanlists[0]->Quantity}}' class='form-control input-sm Quantity' placeholder='100' autocomplete required></td>
							<td><input type='text' name='Amount[]' id='Amount_0' value='{{ 'Rp '. number_format( $permintaanlists[0]->Amount, 0,',', '.' ) }}' class='form-control input-sm Amount' placeholder='Rp 100.000' autocomplete required></td>
							@if(env('APP_TYPE')=='Jual')
								<td><input type='text' name='Type[]' id='Type_0' value='{{$permintaanlists[0]->Type}}' class='form-control input-sm Type' readonly></td>
							@endif
						</tr>
						@for($key=1;$key<count($permintaanlists);$key++)
							<tr class='tr_input'>
								<td><a href='javascript:void(0);' class='remCF glyphicon glyphicon-remove'></a></td>
								<td><input type='text' name='Barang[]' id='{{"Barang_".$key}}' value='{{$permintaanlists[$key]->Barang}}' class='form-control input-sm Barang' placeholder='Main Frame' autocomplete required></td>
								<td><input type='text' name='ICode[]' id='{{"ICode_".$key}}' value='{{$permintaanlists[$key]->ICode}}' class='form-control input-sm ICode' readonly></td>
								<td><input type='number' name='Quantity[]' id='{{"Quantity_".$key}}' value='{{$permintaanlists[$key]->Quantity}}' class='form-control input-sm Quantity' placeholder='100' autocomplete required></td>
								<td><input type='text' name='Amount[]' id='{{"Amount_".$key}}' value='{{ 'Rp '. number_format( $permintaanlists[$key]->Amount, 0,',', '.' ) }}' class='form-control input-sm Amount' placeholder='Rp 100.000' autocomplete required></td>
								@if(env('APP_TYPE')=='Jual')
									<td><input type='text' name='Type[]' id='{{"Type_".$key}}' value='{{$permintaanlists[$key]->Type}}' class='form-control input-sm Type' readonly></td>
								@endif
							</tr>
						@endfor
					</tbody>
